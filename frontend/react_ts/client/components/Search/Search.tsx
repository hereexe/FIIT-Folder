...
export default function SearchMenu({ onClose }: SearchMenuProps) {
  ...
  return (
    <div className="bg-purple-light p-2 sm:p-4 md:p-6 lg:p-8 flex items-start justify-center">
      <div className="w-full max-w-[1200px]">
        <div className="flex flex-col lg:flex-row gap-4 mb-6">
          <div className="flex-1 flex items-center gap-2.5 px-4 py-2.5 rounded-[30px] border-2 border-purple-dark bg-white/50">
            <Search className="w-6 h-6 sm:w-8 sm:h-8 md:w-10 md:h-10 text-purple-dark flex-shrink-0" strokeWidth={2} />
            <input
              type="text"
              placeholder="Search"
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              className="flex-1 bg-transparent text-base sm:text-lg md:text-xl text-purple-dark/75 placeholder:text-purple-dark/75 outline-none font-normal"
            />
          </div>
          ...
          <div className="lg:w-40 flex items-center justify-between px-2.5 py-2.5 rounded-[30px] bg-purple-medium gap-2">
            ...
            >
              <Filter className="w-6 h-6 sm:w-8 sm:h-8 md:w-10 md:h-10 text-purple-dark flex-shrink-0" strokeWidth={2} />
              <div className="relative flex-shrink-0">
                <div className="w-6 h-6 sm:w-8 sm:h-8 md:w-10 md:h-10 rounded-full bg-white/50 flex items-center justify-center">
                  <span className="text-purple-dark/75 text-base sm:text-lg md:text-2xl font-medium">
                    {getFilterCount()}
                  </span>
                </div>
              </div>
            ...
            {showFilters ? (
              ...
              >
                <CheckIcon className="w-6 h-6 sm:w-8 sm:h-8 md:w-10 md:h-10 text-purple-dark" strokeWidth={2} />
              </button>
            ) : (
              <button
                onClick={onClose}
                className="flex-shrink-0 hover:opacity-70 transition-opacity"
              >
                <X className="w-6 h-6 sm:w-8 sm:h-8 md:w-10 md:h-10 text-purple-dark" strokeWidth={2} />
              </button>
            )}
          </div>
        </div>
        ...
        {showFilters && (
          <div className="relative bg-purple-medium rounded-[20px] p-4 sm:p-6 md:p-8">
            <button
              onClick={() => setShowFilters(false)}
              className="absolute top-4 right-4 hover:opacity-70 transition-opacity"
              aria-label="Close filters"
            >
              <X className="w-6 h-6 sm:w-8 sm:h-8 text-purple-dark" strokeWidth={2} />
            </button>
            <div className="space-y-4 sm:space-y-6 md:space-y-8">
              <div className="space-y-4">
                <h2 className="text-lg sm:text-xl md:text-2xl font-medium text-purple-dark tracking-wide">
                  Предмет
                </h2>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {uniqueSubjects.map((subject: SubjectDto) => (
                      <button
                        ...
                        className={`px-2 py-1 sm:py-1.5 rounded-[20px] text-sm sm:text-base md:text-xl font-normal transition-all hover:bg-purple-light/77 ${isSubjectSelected(subject.name) ? "bg-purple-dark text-purple-light" : "bg-white/50 text-purple-dark"}`}
                      >
                        {subject.name}
                      </button>
                    ))}
                ...
                <div className="flex flex-wrap gap-2">
                  {uiContentTypes.map((type) => (
                    <button
                      ...
                      className={`px-2 py-1 sm:py-1.5 rounded-[20px] text-sm sm:text-base md:text-xl font-normal transition-all hover:bg-purple-light/77 ${getButtonStyle(
                        "contentTypes",
            ...
            >
              <RefreshCcw className="w-8 h-8 sm:w-10 sm:h-10 md:w-12 md:h-12 text-purple-dark" strokeWidth={2} />
            </button>
